...
public interface SourceBehaviorConfig {
   ...
   * <p>
   * For example, if you want to write messages from collection "collection-a" to topic "topic1",
   * and messages from collection "collection-b" to topic "topic2", you would write:
  ...
  static ConfigDef.Validator collectionToTopicValidator() {
    return ConfigHelper.validate(CollectionTopicMap::parseCollectionToTopic,
        "collection=topic,...");
  }
   ...
   * <p>
   * Database and collection lists are joined by equal sign.Enabled collections are joined by pipe
   * sign. Map entries are delimited by commas. For example, if you want to enable collections
   * "collection-a" and "collection-b" in database "database-a", and collection-a in database-b, you
   * would write: "database-a=collection-a|collection-b,database-b-=collection-a". To enable all
   * collections in a database, use the wildcard character "*". To enable a collection in all
   * databases, use keyword "__all__" as database name
   * <p>
   */
  @Default
  ...
  /**
   * A map with Collection definitions. A collection is the definition of a type of data
   * <p>
   * Collection definitions are composed of a name and a JMeshpath filter joined by an equal sign.
   * Map entries are delimited by commas. For example if you want to define a collection named
   * "collection-a" containing all documents of of type "type-a" you would write:
   * "collection-a='doc_type==`type-a`'".
   */
  @Default
  List<String> collections();

  @SuppressWarnings("unused")
  static ConfigDef.Validator collectionsValidator() {
    return ConfigHelper.validate(CollectionFilterMap::parseCollectionToFilter,
        "collection-a=doc_type==`type-a`,...");
  }
